/* ============================================================
   ...
   Adapted for LNMIIT UG Physics Lab Hero Section
   ============================================================ */

function initSplashCursor(canvasId, containerId) {
  ...
  const DYE_RESOLUTION = isMobile ? 512 : 1024;
  ...
  const CURL = 3;
  const SPLAT_RADIUS = isMobile ? 0.35 : 0.2;
  ...
  function generateColor() {
    // Dark Science aesthetic (Neon Blue, Cyan, Deep Violet)
    let hue = 0.5 + Math.random() * 0.35;
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      ...
      if (isActive) {
        ...
        // Keep the WebGL context intact, but clean up the continuous tracking
        detachEvents();
        // We DON'T set animationFrameId to null here, we let the inner loop exit naturally and set itself to null
        // to avoid any race conditions stopping and starting the animation instantly.
      }
    });
